...
# Variables
MODELS_PROD = "models_prod/lendingclub.pkl"
INPUTS = {
    "annual_inc_bin": 160000,
    "loan_amnt_bin": 10000,
    "int_rate_bin": 6.58,
    "purpose": "car",
    "grade": "C",
...
def predict_score(raw_data, points_map_dict, num_columns):
    ...
    points = transform_points(
        raw_data=raw_data, points_map_dict=points_map_dict, num_cols=num_columns
    )
    # print(points.T)
...
def inference(input):
    ...
    # load model
    scorecards = pd.read_pickle(MODELS_PROD)
    points_map_dict = get_points_map_dict(scorecards=scorecards)
    # print(points_map_dict)
    input_table = pd.DataFrame(input, index=[0])
    input_score = predict_score(
        raw_data=input_table, points_map_dict=points_map_dict, num_columns=num_columns
    )
    return input_score
