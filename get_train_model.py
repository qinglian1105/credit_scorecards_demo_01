...
num_clns = ['annual_inc', 'loan_amnt', 'int_rate',] 
...
def get_pg_engine(db_name, db_host, db_user, db_pwd, db_port):
    e = create_engine(f'postgresql://{db_user}:{db_pwd}@{db_host}:{db_port}/{db_name}', echo=False)
    return e
...
def create_binning(data, predictor_label, num_of_bins):       
        ...
        data[predictor_label+"_bin"]=pd.qcut(data[predictor_label].rank(method='first'), 
                                             q = num_of_bins)
        return data
...
def main():
    ...
    woe_train = transform_woe(raw_data = X_train, WOE_dict = WOE_map_dict, num_cols = num_clns)    
    ...
    scorecards.to_pickle('models_dev/lendingclub.pkl')
